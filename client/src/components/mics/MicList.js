...
class MicList extends React.Component {
  ...
  renderMics() {
    ...
    return this.props.events.map(mic => {
      return (
        <Table.Row key={mic.name}>
          <Table.Cell collapsing>{mic.name}</Table.Cell>
          <Table.Cell collapsing>{mic.description}</Table.Cell>
          <Table.Cell>{mic.venue.name}</Table.Cell>
          <Table.Cell>
    ...
    return (
      <div>
        <Table celled striped>
          ...
          <Table.Body>{this.renderMics()}</Table.Body>
        </Table>
      </div>
    );
  }
