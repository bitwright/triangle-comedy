...
class ShowList extends React.Component {
  ...
  renderShows() {
    ...
    window.venues = this.props.venues;
    window.events = this.props.events;

    return this.props.events.map(show => {
      return (
        <Table.Row key={show.name}>
          <Table.Cell collapsing>{show.name}</Table.Cell>
          <Table.Cell collapsing>{show.description}</Table.Cell>
    ...
    return (
      <div>
        <Table celled striped>
          ...
          <Table.Body>{this.renderShows()}</Table.Body>
        </Table>
      </div>
    );
  }
